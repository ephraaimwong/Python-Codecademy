...
processed_docs = [preprocess_text(i) for i in documents]
...
# get the index of the most similar response to the user message from documents:
    # object with highest similarity score is stored at index -2. The user_message itself will always have the highest similarity score, and will always be stored at index -1.
similar_response_index = cosine_similarities.argsort()[0][-2]#.argsort() creates new list with indexes of original list sorted in order, NOT rearrange list

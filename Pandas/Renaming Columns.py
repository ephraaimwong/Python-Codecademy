...
imdb.csv 
    ...
    178,Les Misrables,romance,2012,7.6
...
#df.columns = [rename columns in order]
df.columns = ["ID", "Title", "Category", "Year Released", "Rating"]
print(df)

OR
#df.rename(columns={}) will create a new table, so inplace=True necessary to overwrite original. rename() can also specify individual changes unlike .columns()
df.rename(columns={
  "name": "movie_title"
},
inplace=True)
